();; (*- : unit = () *)
2 + 5 * 3;; (* - : int = 17 *)
(* 1.0 * 2 ;; Error, diferencia entre tipos float e int*)
1 * 2 ;;
(* 2 - 2.0;; Error, diferencia entre tipos int y float *)
2 - 2 ;;
(* 3.0 + 2.0 Error, operador tipo int, valores de tipo float*) 
3.0 +. 2.0;;(* - : float = 5.0 *)
5 / 3;;(* - : int = 1*)
5 mod 3;;(* - : int = 2*)
3.0 *. 2.0 ** 3.0;;(* - : float = 24. *)
3.0 = float_of_int 3;;(* - : bool = true*)
(*sqrt 4;; Error, la funcion sqrt es de tipo float, y el valor introducido es int*)
sqrt 4.;;(* - : int = 2*)
int_of_float 2.1 + truncate (-2.9);;(* - : int = 0 *)
truncate 2.1 + truncate (-2.9);;(* - : int = -1*)
floor 2.1 +. floor (-2.9);;(* - : float = -1 *)
(* ceil 2.1 +. ceil -2.9;; Error, faltan parentesis para pasar el float negativo a la funcion ceil *)
ceil 2.1 +. ceil (-2.9);;
...
(*"1999" + "1";;  Error de tipos, entre strings y el operador, de tipo entero *)
"1999" ^ "1";;(* - : string = "19991"*)
int_of_string "1999" + 1;;(* - : int = 2000*)
"\064\065";;(* - : string = "@A" *)
string_of_int 010;;(* - : string = "10" *)

(* not true;  Warning, se espera una expresion, no un bool- : bool = false *)
...
(*if 3=4 then 0 else "4";; Error, no se esperaba el tipo string, se esperaba un int*)
(if 3<5 then 8 else 10)+4;; (* - : int = 12 *)
2.0 *. asin 1.0;; (* - : float = 3.141592*)
...
x-y;; (* - : int = -1 *)
(*z;;*) (* Error el elemento z no tiene un tipo definido *)
...
(*l 2;; Error de tipos, l esperaba un float, no un entero *)
l 2.0;;
(*pi;; Error, el valor pi no esta determinado, ni tiene tipo *)
